...
GasDynamicsEquationSystem::GasDynamicsEquationSystem(
  ...
  : EquationSystem(eqSystems, "GasDynamicsEQS", "mixture_fraction"),
    ...
    fakeNorm_(0.0)
{
  // must be elem-based; AUSM+ based; no gradient extrapolation and limiting
  if ( realm_.realmUsesEdges_ )
    throw std::runtime_error("GasDynamicsEquationSystem MUST NOT activate edges");
  ...
  stk::mesh::Part *part)
{
  ...
  if ( it == assembleGasDynAlgDriver_->algMap_.end() ) {
    Algorithm *theAlg = new AssembleGasDynamicsElemFluxAlgorithm(realm_, part, density_, 
...
GasDynamicsEquationSystem::update_gas_dynamics()
{
  ...
  if ( realm_.hasOverset_ ) {
    throw std::runtime_error("GasDynamicsEquationSystem does not support overset");
  }
